...
class WeatherService(
    val httpExchangeService: HttpExchangeService
) {
    ...
    fun getWeather(coordinates: Coordinates): Weather {
        ...
        val jsonNode = httpExchangeService.request(
            "https://api.open-meteo.com/v1/forecast?latitude=${coordinates.latitude}&longitude=${coordinates.longitude}&current=temperature_2m,is_day,precipitation,rain&hourly=uv_index,uv_index_clear_sky&forecast_days=1&forecast_hours=1"
        )
